...
const MOCK_DELAY_MS = Math.max(
	0,
	Number(import.meta.env.VITE_MOCK_DELAY_MS ?? 30),
);
...
function currentMember(request: Request): Member | undefined {
	...
	const memberId = Object.entries(getMockDb().memberKeys).find(
		([, value]) => value === key,
	)?.[0];
...
function requireWorkspace(
	...
	const member = currentMember(request);
	if (!member) return error("访问密钥无效", 401);
	if (!getMockDb().workspaces.some((workspace) => workspace.id === workspaceId))
		return error("工作区不存在", 404);
	if (!memberCanAccessWorkspace(member.id, workspaceId))
		return error("无权访问当前工作区", 403);
}
...
function normalizeColumn(column: BoardColumn): void {
	...
	activeTasks(column).forEach((task, index) => {
		task.position = index;
	});
}
...
function findMilestone(
	milestoneId: string,
): { projectId: string; index: number } | undefined {
	const db = getMockDb();
...
export const handlers = [
	...
	http.post(mswPattern("authVerify"), async ({ request }) => {
		...
		const ok =
			key.length > 0 && Object.values(getMockDb().memberKeys).includes(key);
	...
	http.post(mswPattern("workspaceMembers"), async ({ params, request }) => {
		...
		const list =
			getMockDb().workspaceMembers[workspaceId] ??
			(getMockDb().workspaceMembers[workspaceId] = []);
	...
	http.delete(mswPattern("workspaceMember"), async ({ params, request }) => {
		...
		if (member.role === "admin")
			return error("管理员自动拥有全部工作区权限", 403);
	...
	http.patch(mswPattern("member"), async ({ params, request }) => {
		...
		if (current.role !== "admin" && current.id !== id)
			return error("当前成员没有执行此操作的权限", 403);
		...
		if (member) {
			...
			if (body.name?.trim() && member.name !== previousName)
				recordScopedActivity("member", member.id, "", "member.updated", {
					name: member.name,
				});
	...
	http.patch(mswPattern("memberRole"), async ({ params, request }) => {
		const denied = requireAdmin(request);
		if (denied) return denied;
		const id = textParam(params.id);
		...
		if (body.role !== "admin" && body.role !== "member")
			return error("角色无效", 400);
		if (
			target.role === "admin" &&
			body.role === "member" &&
			getMockDb().members.filter((item) => item.role === "admin").length <= 1
		) {
		...
		if (
			target.role !== "admin" &&
			body.role === "admin" &&
			getMockDb().members.filter((item) => item.role === "admin").length >= 2
		) {
	...
	// 管理员原子转移：目标升为 admin、调用方降为 member（与后端 TransferAdmin 契约一致）。
	http.post(
		mswPattern("memberTransferAdmin"),
		async ({ params, request }) => {
			...
			if (!actor || !targetId)
				return error("只能把管理员身份转移给普通成员", 400);
			if (!transferAdmin(actor.id, targetId))
				return error("只能把管理员身份转移给普通成员", 400);
			const target = getMockDb().members.find((item) => item.id === targetId);
			await mockDelay();
			return HttpResponse.json(target);
		},
	),
	...
	http.post(mswPattern("memberKey"), async ({ params, request }) => {
		...
		if (actor.role !== "admin" && actor.id !== targetId)
			return error("只能轮换自己的密钥，或由管理员管理成员密钥", 403);
	...
	http.delete(mswPattern("memberKey"), async ({ params, request }) => {
		...
		if (!member) return error("成员不存在", 404);
		if (member.role === "admin")
			return error("不能撤销管理员密钥，请改为轮换", 400);
		if (!revokeMemberKey(memberId)) return error("撤销成员密钥失败", 500);
		recordScopedActivity("member", memberId, "", "member.key_revoked", {
			name: member.name,
		});
		return new HttpResponse(null, { status: 204 });
	}),
	...
	http.post(mswPattern("members"), async ({ request }) => {
		...
		if (getMockDb().members.length >= MEMBER_LIMIT)
			return error("成员和管理员总数已达上限（6 人）", 409);
		const result = createMember(name);
		if (!result.ok) return error(result.error, 400);
		recordScopedActivity("member", result.member.id, "", "member.created", {
	...
	http.delete(mswPattern("member"), async ({ params, request }) => {
		...
		if (!result.ok) return error(result.error, 400);
		if (member) {
			recordScopedActivity("member", member.id, "", "member.deleted", {
				name: member.name,
			});
			persistMockDb();
		}
	...
	http.get(mswPattern("workspaces"), async ({ request }) => {
		...
		const workspaces =
			member.role === "admin"
				...
				: getMockDb().workspaces.filter((workspace) =>
						memberCanAccessWorkspace(member.id, workspace.id),
					);
	...
	http.post(mswPattern("workspaces"), async ({ request }) => {
		...
		const body = (await request.json()) as { name?: string };
		const workspace = {
			id: newMockId("workspace"),
			name: body.name?.trim() || "新工作区",
			createdAt: now(),
		};
		const db = getMockDb();
		db.workspaces.push(workspace);
		const defaultProject = {
			id: newMockId("project"),
		...
		const columns: BoardColumn[] = columnNames.map((name, position) => ({
			id: newMockId("column"),
	...
	http.patch(mswPattern("workspace"), async ({ params, request }) => {
		const denied = requireAdmin(request);
		if (denied) return denied;
		const workspace = getMockDb().workspaces.find(
			(item) => item.id === textParam(params.id),
		);
	...
	http.delete(mswPattern("workspace"), async ({ params, request }) => {
		...
		for (const project of db.projects[workspaceId] ?? []) {
			for (const task of db.boards[project.id]?.columns.flatMap(
				(column) => column.tasks,
			) ?? [])
				delete db.details[task.id];
	...
	http.post(mswPattern("workspaceProjects"), async ({ params, request }) => {
		...
		if (denied) return denied;
		const workspaceId = textParam(params.workspaceId);
		if (!getMockDb().workspaces.some((item) => item.id === workspaceId))
			return error("工作区不存在", 404);
		const body = (await request.json()) as { name?: string };
		...
		getMockDb().boards[project.id] = {
			project,
			columns,
			labels: getMockDb().labels[project.id] ?? [],
		};
		getMockDb().milestones[project.id] = [];
		recordScopedActivity("project", project.id, project.id, "project.created", {
			name: project.name,
		});
	...
	http.patch(mswPattern("project"), async ({ params, request }) => {
		...
		const denied = requireAdmin(request) ?? requireProject(request, projectId);
		if (denied) return denied;
		const project = findProject(projectId)!;
		const body = (await request.json()) as { name?: string };
		...
		for (const detail of Object.values(getMockDb().details))
			if (detail.task.projectId === project.id) detail.projectName = project.name;
		if (body.name?.trim())
			recordScopedActivity("project", project.id, project.id, "project.updated", {
				name: project.name,
			});
	...
	http.delete(mswPattern("project"), async ({ params, request }) => {
		...
		const project = findProject(projectId)!;
		const db = getMockDb();
		db.projects[project.workspaceId] = (
			db.projects[project.workspaceId] ?? []
		).filter((item) => item.id !== projectId);
		delete db.boards[projectId];
		delete db.milestones[projectId];
		for (const taskId of Object.keys(db.details))
			if (db.details[taskId].task.projectId === projectId)
				delete db.details[taskId];
		db.activities = db.activities.filter(
	...
	http.get(mswPattern("project"), async ({ params, request }) => {
		...
		return HttpResponse.json({
			...
			columns: value.columns.map((column) => ({
				...column,
				tasks: activeTasks(column),
			})),
		});
	}),

	http.get(mswPattern("dashboard"), async ({ params, request }) => {
		await mockDelay();
		const denied = requireWorkspace(request, textParam(params.workspaceId));
		if (denied) return denied;
		return HttpResponse.json(dashboard(textParam(params.workspaceId)));
	}),
	http.get(mswPattern("activity"), async ({ params, request }) => {
		await mockDelay();
		const denied = requireWorkspace(request, textParam(params.workspaceId));
		if (denied) return denied;
		return HttpResponse.json(activities(textParam(params.workspaceId)));
	}),
		...
		const tasks = Object.values(getMockDb().boards)
			...
			.flatMap((item) =>
				item.columns.flatMap((column) =>
					column.tasks
						.filter((task) => !task.archivedAt && task.dueDate)
						.map((task) => ({
							...task,
	...
	http.get(mswPattern("search"), async ({ request, params }) => {
		await mockDelay();
		const denied = requireWorkspace(request, textParam(params.workspaceId));
		if (denied) return denied;
		return HttpResponse.json(
			search(
				...
				textParam(params.workspaceId),
			),
		);
	}),
	http.get(mswPattern("settingsBackup"), async ({ request }) => {
		const denied = requireAdmin(request);
		if (denied) return denied;
		await mockDelay();
	...
	http.post(mswPattern("projectColumns"), async ({ params, request }) => {
		...
		recordScopedActivity("column", column.id, projectId, "column.created", {
			name: column.name,
		});
		return HttpResponse.json(persistAnd({ ...column, tasks: undefined }), {
			status: 201,
		});
	}),
	http.patch(mswPattern("column"), async ({ params, request }) => {
		...
		if (typeof body.position === "number") {
			const from = hit.board.columns.findIndex(
				(column) => column.id === hit.column.id,
			);
			hit.board.columns.splice(from, 1);
			hit.board.columns.splice(
				Math.min(Math.max(body.position, 0), hit.board.columns.length),
				0,
				hit.column,
			);
			hit.board.columns.forEach((column, index) => {
		...
		recordScopedActivity(
			"column",
			hit.column.id,
			hit.board.project.id,
			typeof body.position === "number" ? "column.moved" : "column.updated",
			{
				name: hit.column.name,
				...(typeof body.position === "number"
					? {}
					: { wipLimit: hit.column.wipLimit }),
			},
		);
		persistMockDb();
	...
	http.delete(mswPattern("column"), async ({ params }) => {
		...
		hit.board.columns = hit.board.columns.filter(
			(column) => column.id !== hit.column.id,
		);
		hit.board.columns.forEach((column, index) => {
			column.position = index;
	...
	http.post(mswPattern("columnTasks"), async ({ params, request }) => {
		...
		return HttpResponse.json(persistAnd(createTask(hit.column, body)), {
			status: 201,
		});
	}),
	http.patch(mswPattern("task"), async ({ params, request }) => {
		...
		if (moved) {
			...
			const lastColumnId = [...(currentBoard?.columns ?? [])]
				.sort((a, b) => a.position - b.position)
				.at(-1)?.id;
			task.completedAt =
				lastColumnId && task.columnId === lastColumnId ? now() : null;
		}
		syncTask(task);
		recordTaskActivity(
			task,
			...
			moved
				? {
						...
						toName: getMockDb().boards[task.projectId]?.columns.find(
	...
	http.delete(mswPattern("task"), async ({ params }) => {
		...
		db.activities = db.activities.filter(
			(activity) => activity.resourceId !== found.task.id,
		);
		persistMockDb();
		return new HttpResponse(null, { status: 204 });
	}),
	http.post(mswPattern("taskArchive"), async ({ params }) =>
		archiveTask(textParam(params.id), true),
	),
	http.post(mswPattern("taskRestore"), async ({ params }) =>
		archiveTask(textParam(params.id), false),
	),
	http.get(mswPattern("projectArchivedTasks"), async ({ params }) => {
		const currentBoard = getMockDb().boards[textParam(params.id)];
		if (!currentBoard) return error("项目不存在", 404);
		return HttpResponse.json(
			currentBoard.columns.flatMap((column) =>
				column.tasks.filter((task) => task.archivedAt),
			),
		);
	}),
	...
	http.post(mswPattern("taskComments"), async ({ params, request }) => {
		...
		const activity = {
			...
			data: JSON.stringify({ content: comment.content }),
			createdAt: comment.createdAt,
		};
		detail.activity.unshift(activity);
		getMockDb().activities.unshift({
			...activity,
	...
	http.delete(mswPattern("comment"), async ({ params }) => {
		const commentId = textParam(params.id);
		for (const detail of Object.values(getMockDb().details)) {
			const index = detail.comments.findIndex(
			...
			if (index >= 0) {
				...
				const activity = {
					...
					actor: "Admin",
					projectName: detail.projectName,
					data: JSON.stringify({ content: detail.comments[index].content }),
					createdAt: now(),
				};
	...
	http.post(mswPattern("projectLabels"), async ({ params, request }) => {
		...
		const label: Label = {
			id: newMockId("label"),
			projectId,
			name: body.name?.trim() || "新标签",
			createdAt: now(),
		};
		const projectLabels = (getMockDb().labels[projectId] ??= []);
		projectLabels.push(label);
		if (getMockDb().boards[projectId])
			getMockDb().boards[projectId].labels = projectLabels;
		recordScopedActivity("label", label.id, projectId, "label.created", {
			name: label.name,
		});
	...
	http.patch(mswPattern("label"), async ({ params, request }) => {
		...
		for (const task of Object.values(getMockDb().boards).flatMap((item) =>
			item.columns.flatMap((column) => column.tasks),
		))
			task.labels = task.labels?.map((label) =>
				label.id === found.label.id ? found.label : label,
			);
		recordScopedActivity(
			"label",
			found.label.id,
			found.label.projectId,
			"label.updated",
			{ name: found.label.name },
		);
	...
	http.delete(mswPattern("label"), async ({ params }) => {
		...
		for (const board of Object.values(getMockDb().boards)) {
			board.labels = board.labels.filter((label) => label.id !== found.label.id);
			for (const task of board.columns.flatMap((column) => column.tasks)) {
				task.labels = task.labels?.filter((label) => label.id !== found.label.id);
				syncTask(task);
			}
		}
		persistMockDb();
	...
	http.post(mswPattern("taskLabels"), async ({ params }) =>
		toggleLabel(textParam(params.taskId), textParam(params.labelId), true),
	),
	http.delete(mswPattern("taskLabels"), async ({ params }) =>
		toggleLabel(textParam(params.taskId), textParam(params.labelId), false),
	),
	...
	http.get(mswPattern("projectMilestones"), async ({ params }) => {
		...
		const lastColumnId =
			[...currentBoard.columns].sort((a, b) => a.position - b.position).at(-1)
				?.id ?? "";
		return HttpResponse.json(
			(db.milestones[projectId] ?? []).map((milestone) => {
				const linked = currentBoard.columns
					.flatMap((column) => column.tasks)
					.filter((task) =>
						(db.taskMilestones[task.id] ?? []).includes(milestone.id),
					);
				return {
					...milestone,
					progress: {
						done: linked.filter(
							(task) => !task.archivedAt && task.columnId === lastColumnId,
						).length,
						total: linked.length,
					},
				};
	...
	http.post(mswPattern("projectMilestones"), async ({ params, request }) => {
		...
		const milestone = {
			id: newMockId("milestone"),
			projectId,
			name: body.name?.trim() || "新里程碑",
	...
	http.post(mswPattern("taskMilestones"), async ({ params }) =>
		milestoneLink(textParam(params.taskId), textParam(params.milestoneId), true),
	),
	http.delete(mswPattern("taskMilestones"), async ({ params }) =>
		milestoneLink(textParam(params.taskId), textParam(params.milestoneId), false),
	),
...
async function archiveTask(taskId: string, archived: boolean) {
	...
	syncTask(found.task);
	recordTaskActivity(found.task, archived ? "task.archived" : "task.restored", {
		archivedAt: found.task.archivedAt,
...
async function toggleLabel(taskId: string, labelId: string, attach: boolean) {
	...
	found.task.labels = attach
		? [...(found.task.labels ?? []).filter((item) => item.id !== label.id), label]
		: (found.task.labels ?? []).filter((item) => item.id !== label.id);
	syncTask(found.task);
	recordTaskActivity(found.task, attach ? "label.attached" : "label.detached", {
		label: label.name,
	});
	persistMockDb();
	return new HttpResponse(null, { status: 204 });
...
async function milestoneLink(
	...
	if (task.task.projectId !== milestone.projectId)
		return error("任务与里程碑不属于同一项目", 400);
	const links = (getMockDb().taskMilestones[taskId] ??= []);
	if (attach && !links.includes(milestoneId)) links.push(milestoneId);
	if (!attach)
		getMockDb().taskMilestones[taskId] = links.filter((id) => id !== milestoneId);
	// 里程碑关联也记活动（此前漏记），带名称供活动流展示。
	const milestoneName =
		getMockDb().milestones[milestone.projectId]?.[milestone.index]?.name;
	recordTaskActivity(
		task.task,
		attach ? "milestone.attached" : "milestone.detached",
