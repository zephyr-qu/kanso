...
import { useRoutePerf } from "@/hooks/use-route-perf";
import { usePinnedProjects } from "@/lib/pinned-projects";
import { preloadRoute } from "@/lib/route-preload";
import { prefetchBoard, prefetchWorkspaceProjects } from "@/lib/query-prefetch";
...
function AppShellContent() {
	...
	useRoutePerf(location.pathname);
	...
	// 「当前工作区」语义已收口到 context（含展示回退），壳里不再自行推导。
	...
	const { data: currentWorkspaceProjects } = useQuery({
		...
		queryFn: () =>
			api<Project[]>(
				buildPath("workspaceProjects", { workspaceId: currentWorkspaceId }),
			),
			...
			<div data-testid="app-shell" data-kanso-app className="kanso-shell">
				...
				>
					...
					<div data-testid="brand" className="kanso-brand">
						<span className="kanso-brand-mark" aria-hidden>
							簡
						</span>
					...
					<div
						className={`kanso-workspace-switcher${workspaceMenuOpen ? " is-open" : ""}`}
					>
						<button
							type="button"
						...
						>
							<span className="kanso-workspace-switcher-icon" aria-hidden="true">
						...
						{workspaceMenuOpen ? (
							<div
								className="kanso-workspace-switcher-menu"
								role="menu"
								aria-label="切换工作区"
							>
						...
						{currentWorkspaceId ? (
							...
							>
								{member ? (
									<MemberAvatar
										member={member}
										className="size-7 text-[11px] font-semibold text-white"
									/>
								) : (
									<img
								...
								<span className="kanso-sidebar-identity-label min-w-0 flex-1 truncate">
									{member?.name ?? "未登录"}
								</span>
							</NavLink>
						) : (
				...
				<main data-testid="main-region" className="kanso-main">
					{workspaceContext.isRouteAuthoritative ||
					workspaceContext.status === "not-required" ? (
						<Outlet />
					) : (
